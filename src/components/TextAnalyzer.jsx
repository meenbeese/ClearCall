...
const TextAnalyzer = ({ text }) => {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(async () => {
      ...
      if (!currentText || currentText === lastAnalyzedText.current || isAnalyzing) return;
  ...
  return (
    <div className="fixed right-4 top-4 bottom-4 w-80 bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden flex flex-col border border-gray-200 dark:border-gray-700">
      <div className="p-4 border-b border-gray-200 dark:border-gray-700">
        ...
        {isAnalyzing && <span className="text-sm text-blue-500">Analyzing conversation...</span>}
      </div>
        ...
        {analysisResults.map((result, index) => (
          <div key={index} className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg space-y-2">
            <div className="flex justify-between items-center">
              ...
              <span className="px-2 py-1 text-xs bg-red-100 text-red-800 rounded-full">Alert</span>
            </div>

            <div className="text-sm">
              <div className="font-medium text-gray-600 dark:text-gray-300">Analyzed Text:</div>
            ...
            <div className="text-sm">
              <div className="font-medium text-gray-600 dark:text-gray-300">Analysis:</div>
              <div className="text-red-600 dark:text-red-400">{result.analysis}</div>
            </div>
          </div>
        ))}
